...
@Component({
  standalone: true,
  imports:[ CommonModule, MatModuleModule,ModbusAttributeFormComponent],
...
export class ModbusDeviceAttributesComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  getModbusAttributeDevice(params: string): void {
    this.subs.add(this.modbusDeviceAttributesService.get(params).subscribe((data: any) => {
  ...
  copyData(params:any){
    this._commonService.openConfirmDialog('Would you like to copy',
      ...
      if (response) {
    this.subs.add(this.modbusDeviceAttributesService.copy(params.id).subscribe((data: any) => {
      this._commonService.notification(this.copyMsg);
      const param = 'limit(' + this.limit + ',' + this.offset + ')&sort(+name))';
      this.getModbusAttributeDevice(param);
      }, (error: { result: { message: any[]; }; }) => {
  ...
  delete(element: any){
    this._commonService.openConfirmDialog('Would you like to delete',
      element.name).afterClosed().subscribe((response: any) => {
      if (response) {
        this.subs.add(this.modbusDeviceAttributesService.delete(element.id).subscribe((data: any) => {
          const param = 'limit(' + this.limit + ',' + this.offset + ')&sort(+name))';
          this.getModbusAttributeDevice(param);
