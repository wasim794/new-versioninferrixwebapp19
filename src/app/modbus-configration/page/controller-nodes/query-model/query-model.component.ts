...
export class QueryModelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  selectedQueryList() {
    this.queryOptions = [];
    this.queryListOptions.queryAllList.mappings.forEach((value, index) => {
  ...
  updateQuery() {
    this.queryListOptions.queryAllList.mappings.forEach(data=>{
      this.arrayFilter.push(data.modbusQueryId);
    });
    let missing = this.queryOptions.
    filter(item => this.arrayFilter.indexOf(item) < 0);
